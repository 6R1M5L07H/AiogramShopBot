...
- `PAID` (digital only)
- `PAID_AWAITING_SHIPMENT` ⚠️ **REQUIRES SHIPPING ACTION**
- `SHIPPED`
- `CANCELLED_BY_USER`
- `CANCELLED_BY_ADMIN`
- `CANCELLED_BY_SYSTEM`
...
**Filter groups:**
- **Requires Action** (PAID_AWAITING_SHIPMENT) - Default filter! ⚠️
- All orders
...
Rules:
- `PAID_AWAITING_SHIPMENT`: **Mark as Shipped** button (primary action) ⚠️
...
- [ ] Add new admin strings:
  ...
  - `admin_order_requires_action` ("⚠️ Requires Shipping")
...
- [ ] Add filter group labels:
  - `admin_order_filter_requires_action` ("⚠️ Requires Action")
...
- [ ] Implement `show_order_filters()`:
  - **"⚠️ Requires Action" at the top (PAID_AWAITING_SHIPMENT)**
...
- [ ] Filter button:
  ...
  - **"⚠️ Requires Action" for PAID_AWAITING_SHIPMENT filter**
  - Navigate to Level -1
